...
def test_import_voitings():
    ...
    assert Voting.objects.get(vid=31213).title == (
        "DĖL VILNIAUS MIESTO SAVIVALDYBĖS KONTROLIUOJAMŲ AKCINIŲ BENDROVIŲ IR UŽDARŲJŲ AKCINIŲ BENDROVIŲ STEBĖTOJŲ "
        "TARYBŲ SUDĖČIŲ TVIRTINIMO"
    )
    assert Voting.objects.get(vid=31214).title.startswith("DĖL VILNIAUS MIESTO SAVIVALDYBĖS")
    ...
    assert Vote.objects.count() == 5
    assert Vote.objects.filter(voting__vid=31213).count() == 3
    assert Vote.objects.filter(voting__vid=31214).count() == 1
