...
from manopozicija.website.parsers.names import parse_index, parse_names, strip_accents


class ParseNamesTests(unittest.TestCase):

    def test_parse_index(self):
        html = pres.resource_string('manopozicija.website.tests', 'fixtures/names_index.html')
        result = parse_index(html)
        self.assertEqual(result[0], 'http://vardai.vlkk.lt/sarasas/a/')
        self.assertEqual(len(result), 25)

    def test_parse_names(self):
        html = pres.resource_string('manopozicija.website.tests', 'fixtures/names_page.html')
