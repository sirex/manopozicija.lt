...
def test_curator_apply(app):
    ...
    resp = app.post(reverse('js:curator-post-vote', args=[post.pk]), params={'vote': 1}, user=curator)
    assert resp.json == {'success': True, 'upvotes': 1, 'downvotes': 0}
    assert services.dump_topic_posts(topic, queue=True) == ''

    # A new curator user tries to vote for his own curator application
    resp = app.post(reverse('js:curator-post-vote', args=[post.pk]), params={'vote': 1}, user=user)
...
def test_user_post_vote(app):
    ...
    # User upvotes a post
    resp = app.post(reverse('js:user-post-vote', args=[post.pk]), params={'vote': 1}, user=user)
    assert resp.json == {'success': True, 'upvotes': 1, 'downvotes': 0}
    assert services.dump_topic_posts(topic) == '\n'.join([
    ...
    resp = app.post(reverse('js:user-post-vote', args=[post.pk]), params={'vote': 0}, user=user)
